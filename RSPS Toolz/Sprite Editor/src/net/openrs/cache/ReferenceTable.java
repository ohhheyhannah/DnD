...
public class ReferenceTable {
	...
	public static ReferenceTable decode(ByteBuffer buffer) {
		...
		/* read the ids */
		int[] ids = new int[table.format >= 7 ? ByteBufferUtils.getSmartInteger(buffer) : buffer.getShort() & 0xFFFF];
		int accumulator = 0, size = -1;
		for (int i = 0; i < ids.length; i++) {
			int delta = table.format >= 7 ? ByteBufferUtils.getSmartInteger(buffer) : buffer.getShort() & 0xFFFF;
		...
		for (int id : ids) {
			members[id] = new int[table.format >= 7 ? ByteBufferUtils.getSmartInteger(buffer) : buffer.getShort() & 0xFFFF];
		}
	...
	/**
	 * Gets the entries map
	 * @return The sorted map.
	 */
	public SortedMap<Integer, Entry> getEntries() {
		return entries;
	}
